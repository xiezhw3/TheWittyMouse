...
void LoadingScene::loadRes() {
	...
	frameCache->addSpriteFramesWithFile("images/begin.plist", "images/begin.png");
	frameCache->addSpriteFramesWithFile("images/scotter.plist", "images/scotter.png");
...
void LoadingScene::loadData() {
	...
	if (reader.parse(data, root, false)) {
		...
		Util::setDistanceTravelled(root["distance"].asInt());
		Util::setInitMouseLife(root["life"].asInt());
	}
	else {
		Mouse::setIQ(INIT_IQ);
		Mouse::setLife(INIT_LIFE);
		Util::setDistanceTravelled(INIT_DISTANCE);
		Util::setInitMouseLife(INIT_LIFE);
	}
}